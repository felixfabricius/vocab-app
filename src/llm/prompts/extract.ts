 ...
 *    fenced JSON block headed VOCABAPP-IMPORT v1 that `parsePasteImport` reads
 */
import { VARIETY } from "@/config/variety";
import type { Chunking } from "@/core/types";

export const PASTE_HEADER = "VOCABAPP-IMPORT v1";
...
export function buildExtractPrompt(o: ExtractPromptOptions): string {
  ...
  const cap = o.maxItems ? `Return at most ${o.maxItems} items, the most useful first.` : "Return every vocabulary item on the page or in the text; do not cap.";
  const lines: string[] = [];
  ...
  lines.push(
    ...
    "",
    "## Items",
    "One item per lemma + part of speech. The lemma is the dictionary form: infinitive for verbs, masculine singular for nouns and adjectives. Inflected forms found in the input are recorded as `sourceSentence.target`, never as separate items.",
    ...
    "",
    "## Sentences",
    "If the item appeared in a sentence in the input, copy that sentence verbatim into `sourceSentence` with an English translation and `target` = the exact form as it appears.",
    "Always write one `generatedSentence`: 6–12 words, natural, everyday register, shows the item in its most typical use. Prefer common words; do not restrict yourself to the known list. Do not insert regional slang into sentences unless the item itself is regional. For verbs use a common conjugated form and report it in `verbForm`. Set `target` to the exact form of the item inside the sentence.",
    "For every other content word (noun, verb, adjective, adverb) in the generated sentence that is NOT in the known list, add a minimal entry to `fromSentence`: lemma, pos, one-line gloss, and `target` = its form in the sentence. Function words (articles, prepositions, pronouns, conjunctions) are not listed.",
    "",
    "## Notes",
  ...
  if (o.format === "paste") {
    lines.push(
      ...
      "```",
      "Every field must be present; use null when not applicable. `pos` is one of noun, verb, adj, adv, phrase, prep, conj, pron, interj, num, other. `priority` is essential, core, standard or niche.",
    );
  }
