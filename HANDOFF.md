...
- Speech recognition: native on-device (`SpeechPlugin`) for dictation and sí/no grading; Whisper via `openaiKey` remains the web path. Voice review level 1 runs with the screen on.
- Locked screen: `UIBackgroundModes: audio` and the remote command centre are in place, but whether JavaScript keeps running while locked is unverified (tracker T3/T4).
...
- Native phase progress (2026-09-22): M1 (wrapper, CI, TestFlight) built and uploaded; its phone checklist is still to be ticked. M2 (Again/Good only, scroll containment with `scrollEnabled: false`, tags with Dexie v2, study by tag, flat drafts table with swipe-to-remove, jobs with progress + cancel) and M3 (first Swift plugins `Speech` and `Translate` registered in `ViewController.swift`, added to the project in CI by `scripts/ios/sync-project.rb`; offline translate with pack download in Settings; live on-device dictation; lookups table + "Create cards" with automatic enrichment of drafts via `enrichBatchDrafts`; manual add with "Complete with AI") are implemented but not yet verified on the phone. M4 (prompt builders series/textbook/book/context in `src/llm/prompts/builders.ts`, wire format `VOCABAPP-IMPORT v2` with one `sentence` + `sentenceSource` per item and a top-level `tag`; v1 pastes still parse through `LegacyEntryDraftSchema`) and M5 (`CloudFilesPlugin.swift` + iCloud entitlements; `src/storage/cloudSync.ts` drains the outbox into `changes/*.jsonl`, writes `snapshot.json` on background when due, restores or merges on launch; Settings → iCloud Drive card) are built and on TestFlight (2026-09-22, after switching the App ID's iCloud mode to "Include CloudKit support"). M6 (deep links `alaluna://translate?dir=…` via `src/native/deepLinks.ts`; xcodegen `ios/App/project.yml` with the `ALaLunaWidgets` extension: lock-screen and home widgets plus two Controls in `ios/Widgets/`) is implemented; the Shortcuts path is kept until the widgets are confirmed working on the phone, then removed (spec §1). M7 (`AudioSessionPlugin.swift` with the remote command centre; `src/input/` `InputSource`, `RemoteButtonsInput`, `VoiceInput`; `features/review/voiceLoop.ts` pure loop with tests; Settings playback "Voice review" plus pause/listen seconds) is implemented. All seven milestones are on `main`; the phone checklists for M2–M7 and tracker items T3/T4/T9 remain. Phone feedback so far: build works, Francisca etc. not reported; offline translate and prompt copying were fixed in `ec309cb`.
- Fixed 2026-09-21: the schema bug that broke every Claude call, and the English meaning on both sides of conjugation cards.
- `settings.playback = "handsFree"` is "Voice review" (M7), native only.
